...
class TemplateActivity : Activity() {
    ...
    private fun editTemplate(existing: TextTemplate?) {
        ...
        val sizeInput = EditText(this).apply {
            hint = "字号，例如 72"
            inputType = 2 or 8192
            setText((existing?.textSize ?: 72f).toString())
        }

        val rotationInput = EditText(this).apply {
            hint = "旋转角度，例如 -5"
            inputType = 2 or 4096
            setText((existing?.rotation ?: -5f).toString())
        }
        ...
        wrap.addView(sizeInput)
        wrap.addView(rotationInput)
        wrap.addView(colorLabel)
        wrap.addView(colorSpinner)
